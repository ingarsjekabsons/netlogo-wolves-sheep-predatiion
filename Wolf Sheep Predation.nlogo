...
to move-wolf [ speed ]
  let x min-one-of sheep [ distance myself ]
  ifelse (x != nobody) [
    face x ; lock on target ..
    fd (max list speed x)
  ]
  [
    rt random 50
...
to move-sheep [ speed ]
  let c count (wolves-on neighbors)
  (ifelse
    c > 0 [ ; panic and RUN!
      rt random 50
      lt random 50
      fd speed * 1.3 ; we are in panic mode so we can run fast!
      set energy (energy / 2)
  ]
  c <= 0 [
    ifelse model-version = "sheep-wolves-grass" [
      let x min-one-of (patches in-radius 5) [ distance myself ]
      ifelse (x != nobody) [
        face x ;
        fd (max list speed x)
      ]
...
to reproduce-sheep  ; sheep procedure
  if count (sheep-on neighbors) > 0 [ ; we have somebody to make kids with.. Yay!
    set energy (energy / 2)
